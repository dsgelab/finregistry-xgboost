...
def train_xgboost_fr_skopt():
    ...
    parser.add_argument("--objective",help="Supported objectives: classification (=default), regression.",type=str,default='classification',choices=['classification','regression'])
    ...
    logging.info("objective: "+args.objective)
    ...
    if args.objective=='classification':
        #determine whether class weights are used or not
        ...
        xgb_model = xgb.XGBRegressor(objective='reg:squarederror',seed=args.seed,eval_metric='rmse',**{"tree_method": args.tree_method},n_jobs=args.nproc)
    ...
    #make predictions
    if args.objective=='classification': y_pred = model.predict_proba(X_test)
    elif args.objective=='regression': y_pred = model.predict(X_test)
    del(X_test)
    ...
    if args.objective=='classification':
        ...
        df_test.to_csv(args.outdir+args.varname+"_test_set_xgb_preds.csv.gz",compression="gzip")
    del(df_test)
